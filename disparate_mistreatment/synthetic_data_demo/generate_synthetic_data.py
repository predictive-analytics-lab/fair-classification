...
def generate_synthetic_data(data_type, plot_data=False):
    ...
    def gen_gaussian_diff_size(mean_in, cov_in, z_val, class_label, n):
        ...
        z = (
            np.ones(n, dtype=float) * z_val
        )  # all the points in this cluster get this value of the sensitive attribute
    ...
    if data_type == 1:
        ...
        nv1, X1, y1, z1 = gen_gaussian_diff_size(
            mu1, sigma1, 1, +1, int(n_samples * 1)
        )  # z=1, +
        nv2, X2, y2, z2 = gen_gaussian_diff_size(
            mu2, sigma2, 0, +1, int(n_samples * 1)
        )  # z=0, +
        nv3, X3, y3, z3 = gen_gaussian_diff_size(
            mu3, sigma3, 1, -1, int(n_samples * 1)
        )  # z=1, -
        nv4, X4, y4, z4 = gen_gaussian_diff_size(
            mu4, sigma4, 0, -1, int(n_samples * 1)
        )  # z=0, -
