...
def test_synthetic_data():
    ...
    X, y, x_control = generate_synthetic_data(
        data_type=data_type, plot_data=True
    ...
    sensitive_attrs_to_cov_thresh = {
        "s1": {0: {0: 0, 1: 0}, 1: {0: 0, 1: 0}, 2: {0: 0, 1: 0}}
    ...
    psb.plot_boundaries(
        ...
        [w_uncons, w_cons],
        [acc_uncons, acc_cons],
        [s_attr_to_fp_fn_test_uncons["s1"], s_attr_to_fp_fn_test_cons["s1"]],
        "img/syn_cons_dtype_%d_cons_type_%d.png" % (data_type, cons_type),
    )
    print(
        "\n-----------------------------------------------------------------------------------\n"
    )
    ...
    cons_params[
        "cons_type"
    ] = cons_type  # FNR constraint -- just change the cons_type, the rest of parameters should stay the same
    w_cons, acc_cons, s_attr_to_fp_fn_test_cons = train_test_classifier()
    psb.plot_boundaries(
        X,
        y,
