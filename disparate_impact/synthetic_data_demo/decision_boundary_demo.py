...
def test_synthetic_data():
    ...
    def train_test_classifier():
        ...
        p_rule = ut.print_classifier_fairness_stats(
            [test_score], [correlation_dict_test], [cov_dict_test], sensitive_attrs[0]
        )
    ...
    plot_boundaries(
        w_uncons, w_f_cons, p_uncons, p_f_cons, acc_uncons, acc_f_cons, "img/f_cons.png"
    )

    """ Classify such that we optimize for fairness subject to a certain loss in accuracy """
    apply_fairness_constraints = 0  # flag for fairness constraint is set back to0 since we want to apply the accuracy constraint now
    apply_accuracy_constraint = (
        1  # now, we want to optimize fairness subject to accuracy constraints
    )
    sep_constraint = 0
    ...
    plot_boundaries(
        w_uncons, w_a_cons, p_uncons, p_a_cons, acc_uncons, acc_a_cons, "img/a_cons.png"
    )
    ...
    apply_fairness_constraints = 0  # flag for fairness constraint is set back to0 since we want to apply the accuracy constraint now
    apply_accuracy_constraint = (
        1  # now, we want to optimize accuracy subject to fairness constraints
    )
