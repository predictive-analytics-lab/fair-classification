...
def plot_data(X, y, x_sensitive, w_arr, label_arr, lt_arr, fname, title, group=None):
    ...
    x_lim = [
        min(x_draw[:, -2]) - np.absolute(0.3 * min(x_draw[:, -2])),
        max(x_draw[:, -2]) + np.absolute(0.5 * max(x_draw[:, -2])),
    ]
    y_lim = [
        min(x_draw[:, -1]) - np.absolute(0.3 * min(x_draw[:, -1])),
        max(x_draw[:, -1]) + np.absolute(0.7 * max(x_draw[:, -1])),
    ]
