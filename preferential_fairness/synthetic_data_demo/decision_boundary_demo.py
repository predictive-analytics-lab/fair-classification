...
def print_stats_and_plots(x, y, x_sensitive, clf, fname):
    ...
    acc, _, acc_stats = compute_stats.get_clf_stats(
        dist_arr, dist_dict, y, x_sensitive, print_stats=True
    )
...
def test_synthetic_data():
    ...
    data_type = 1
    X, y, x_sensitive = generate_synthetic_data(
        data_type=data_type, n_samples=1000
    )  # set plot_data to False to skip the data plot
    ...
    (
        ...
        X, y, x_sensitive, test_size=TEST_FOLD_SIZE, random_state=1234, shuffle=False
    )
    ...
    lam = {
        0: 0.01,
        1: 0.01,
    }  # the regularization parameter -- we set small values here, in the paper, we cross validate all of regularization parameters
    for s_attr_val in set(x_sensitive_train):
        idx = (
            x_sensitive_train == s_attr_val
        )  # the index for the current sensitive feature group
        clf = LinearClf(
            loss_function, lam=lam[s_attr_val], train_multiple=False
        )  # for more info, see the clf documentation in the respective file
    ...
    print_stats_and_plots(
        x_test, y_test, x_sensitive_test, clf_merged, "img/unconstrained.png"
    )
    ...
    s_val_to_cons_sum_di = compute_stats.get_sensitive_attr_cov(
        dist_dict
    )  # will need this for applying preferred fairness constraints
    ...
    cons_params[
        "tau"
    ] = 1  # the tau value varies based on the dataset we look at. See the DCCP documentation for details
    cons_params["s_val_to_cons_sum"] = s_val_to_cons_sum_di
    lam = {0: 2.5, 1: 0.01}
    clf = LinearClf(loss_function, lam=lam, train_multiple=True)
    clf.fit(x_train, y_train, x_sensitive_train, cons_params)
    print_stats_and_plots(
        x_test, y_test, x_sensitive_test, clf, "img/preferred_impact.png"
    )
    ...
    clf.fit(x_train, y_train, x_sensitive_train, cons_params)
    print_stats_and_plots(
        x_test, y_test, x_sensitive_test, clf, "img/preferred_both.png"
    )
