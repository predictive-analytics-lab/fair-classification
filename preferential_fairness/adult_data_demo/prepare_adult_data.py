...
def check_data_file(fname):
    ...
    if fname not in files:
        ...
        addr = (
            "http://archive.ics.uci.edu/ml/machine-learning-databases/adult/%s" % fname
        )
...
def load_adult_data(load_data_size=None):
    ...
    attrs_to_ignore = [
        ...
        "sex",
        "fnlwgt",
    ]  # sex is the sensitive feature so we will not use it in classification, we will not consider fnlwght for classification since its computed externally and it highly predictive for the class (for details, see documentation of the adult data)
    ...
    for attr_name in attrs_for_classification:
        ...
        if (
            ...
            for (
                ...
                attr_vals
            ):  # each binarized array of size n (n = num examples in the dataset)
                X.append(bin_val_arr)

    # convert to numpy arrays for easy handline
    X = np.array(X, dtype=float).T
    y = np.array(y, dtype=float)
    for k, v in list(x_control.items()):
