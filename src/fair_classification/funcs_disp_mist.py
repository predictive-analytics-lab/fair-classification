...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if cons_params is None:  # just train a simple classifier, no fairness constraints
        ...
        constraints = get_constraint_list_cov(
            ...
            y,
            x_control,
            cons_params["sensitive_attrs_to_cov_thresh"],
            cons_params["cons_type"],
            w,
        )
...
def get_clf_stats(
    ...
    for s_attr in sensitive_attrs:
        ...
        print_stats = (
            False  # we arent printing the stats for the train set to avoid clutter
        )
...
def get_sensitive_attr_constraint_fpr_fnr_cov(
    model, x_arr, y_arr_true, y_arr_dist_boundary, x_control_arr, verbose=False
):
    ...
    for v in set(x_control_arr):
        ...
        cons_sum_dict[0][v] = sum(np.minimum(0, dist_bound_prod)) * (
            s_val_to_avg[0][v] / len(x_arr)
        )
        cons_sum_dict[1][v] = sum(
