...
def train_classifier(
    x, y, control, sensitive_attrs, mode, sensitive_attrs_to_cov_thresh
):
...
def test_classifier(w, x, y, control, sensitive_attrs):
    ...
    correlation_dict_test = ut.get_correlations(
        None, None, all_class_labels_assigned_test, control, sensitive_attrs
    )
    cov_dict_test = ut.print_covariance_sensitive_attrs(
        None, x, distances_boundary_test, control, sensitive_attrs
    )
...
def main(train_file, test_file, output_file, setting, value):
    ...
    thresh = {}
    if setting == "c":
        thresh = {k: float(value) for (k, v) in x_control_train.items()}
...
def write_adult_data_to_disk():
    ...
    (
        ...
        y_test,
        x_control_test,
    ) = ut.split_into_train_test(X, y, x_control, 0.7)
