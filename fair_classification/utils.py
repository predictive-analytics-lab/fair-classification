...
def train_model(
    ...
    if (
        ...
        def cross_cov_abs_optm_func(weight_vec, x_in, x_control_in_arr):
            cross_cov = (x_control_in_arr - np.mean(x_control_in_arr)) * np.dot(
                weight_vec, x_in.T
            )
...
def compute_cross_validation_error(
    ...
    sep_constraint,
    sensitive_attrs,
    sensitive_attrs_to_cov_thresh_arr,
    gamma=None,
):
    ...
    def train_test_single_fold(
        train_data, test_data, fold_num, output_folds, sensitive_attrs_to_cov_thresh
    ):
...
def print_classifier_fairness_stats(
    ...
    print(
        "Protected/non-protected in +ve class: %0.0f%% / %0.0f%%"
...
def compute_p_rule(x_control, class_labels):
    ...
    non_prot_pos = sum(
        class_labels[x_control == 1.0] == 1.0
...
def get_one_hot_encoding(in_arr):
    ...
    if (
        in_arr.dtype != np.float64
        and not isinstance(in_arr[0], int)
        and in_arr.dtype != np.int64
    ):
        print(str(type(in_arr[0])))
...
def check_accuracy(
    model, x_train, y_train, x_test, y_test, y_train_predicted, y_test_predicted
):
    ...
    def get_accuracy(y, Y_predicted):
        correct_answers = (Y_predicted == y).astype(
            int
...
def get_constraint_list_cov(
    x_train, y_train, x_control_train, sensitive_attrs, sensitive_attrs_to_cov_thresh
):
...
def plot_cov_thresh_vs_acc_pos_ratio(
    ...
    train_acc = []
    test_acc = []
    positive_per_category = defaultdict(
        list
    ...
    p_rule_arr = (
        np.array(positive_per_category[0]) / np.array(positive_per_category[1])
    ) * 100.0
