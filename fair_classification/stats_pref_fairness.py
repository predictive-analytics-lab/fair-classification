    ...
    def check_labels_bin(arr):
        ...
        try:
            if len(set(arr)) == 1:
                ...
                assert (
                    sorted(list(set(arr)))[0] == -1 and sorted(list(set(arr)))[1] == 1
                )
        except:
            traceback.print_exc()
            raise Exception(
...
def get_acc_stats(dist_dict, y, x_sensitive, verbose=False):
    ...
    try:
        assert (
            sorted(list(set(x_sensitive)))[0] == 0
            and sorted(list(set(x_sensitive)))[1] == 1
        )
    except:
    ...
    for s_val in set(x_sensitive):
        ...
        y_true_local = y[idx]
        y_pred_local = sign_bin_clf(
            dist_dict[s_val][s_val]
        )  # predictions with this classifier
        y_pred_local_other = sign_bin_clf(
            dist_dict[s_val][other_val]
        )  # predictions with other group's classifier

        assert (
            y_true_local.shape[0] == y_pred_local.shape[0]
        ...
        if verbose == True:
            ...
            print(
                ...
                % (
                    s_val,
                    acc_stats[s_val][s_val]["frac_pos"],
                    acc_stats[s_val][other_val]["frac_pos"],
                )
            )
...
def get_sensitive_attr_cov(dist_dict):
    ...
    for s_val in list(dist_dict.keys()):
        for w_group in list(dist_dict[s_val].keys()):
...
def scale_data(x_train, x_test):
    ...
    for k in range(x_train.shape[1]):
        idx_binary.append(
            np.array_equal(x_train[:, k], x_train[:, k].astype(bool))
