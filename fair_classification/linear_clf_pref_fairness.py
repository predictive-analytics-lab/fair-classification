    ...
    def fit(self, X, y, x_sensitive, cons_params=None):
        ...
        if cons_params is not None:
            ...
            if cons_type == -1:  # no constraint
                ...
                constraints += self.get_preferred_cons(
                    X, x_sensitive, w, cons_type, cons_params["s_val_to_cons_sum"]
                )
            elif cons_type == 2:
        ...
        try:
            ...
            if (
                ...
                if cons_params.get("mu") is not None:
                    mu = cons_params["mu"]
                if cons_params.get("EPS") is not None:
                    EPS = cons_params["EPS"]
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                self.w[k] = np.array(
                    w[k].value
                ).flatten()  # flatten converts it to a 1d array
        else:
    ...
    def get_distance_boundary(self, X, x_sensitive):
        ...
        if not isinstance(self.w, dict):  # we have one model for the whole data
            ...
            for attr in set(x_sensitive):
                ...
                distance_boundary_arr[idx] = self.decision_function(
                    X_g, attr
